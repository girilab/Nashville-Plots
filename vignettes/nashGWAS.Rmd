...
tissue_label <- data.frame(V1 = c("AdiposeSubcutaneous.csv", "CellsTransformedFibroblasts.csv"),
                           V2 = c("AdiposeSubcutaneous", "CellsTransformedFirbroblasts"))

gwas <- read.table(gwas_location, header= TRUE)
meta <- read_metaXcan_folder(directory = metaxcan_results_location, label = tissue_label)


gwas.obj <- make.valid.object(CHR = gwas$CHR,
                              P = gwas$P,
                              BP = as.numeric(gwas$BP))

meta.obj <- make.valid.object(CHR = as.numeric(meta$CHR),
                              P = meta$pvalue,
